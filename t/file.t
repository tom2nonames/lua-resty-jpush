    ...
    server {
        ...
        location /v3/files/ {
            content_by_lua_block {
                ngx.say('{"file_id":"f123","type":"registration_id"}')
            }
        }
    }
--- config
    location /t {
        content_by_lua_block {
            ...
            ngx.say(cjson.encode(client.file:get("f123")))
        }
    }
--- request
GET /t
--- response_body_like
^.*(?=.*"success":true)(?=.*"file_id":"f123").*$
--- no_error_log
[error]
...
--- config
    location /t {
        content_by_lua_block {
            ...
            ngx.say(cjson.encode(client.file:delete("f123")))
        }
    }
--- request
GET /t
